class Admin
  ...
  def declare_computer_move(move)#, possible_places)
  ...
  def update_computer_game_board(move)#, winning_propsitions)
  ...
  def computer_win#(winning_propositions)
  ...
  def player_win
    ...
    b = a.map { |count_the_items_in_hash| count_the_items_in_hash.count }
    b.each { |number_count| puts '#{$user_name} has won.  Good for you though shame on me. :|' if number_count == 3 }
    b.each { |number_count| exit if number_count == 3 }
  end

  def draw_game_outcome#(possible_places)
